...
npm install -D typescript ts-node-dev @types/express @types/node @types/cors @types/bcrypt @types/jsonwebtoken jest ts-jest supertest @types/jest @types/supertest mongodb-memory-server
npx tsc --init
```
...
- [ ] Configure `tailwind.config.js` content paths to `./index.html` and `./src/**/*.{ts,tsx}`.
...
- [ ] `git add . && git commit -m "chore: scaffold Vite+React+TS+Tailwind frontend, configure Vitest"`
- [ ] `git push`
...
Same Red-Green-Refactor loop and commit template as Phase 3. Keep logging prompts.
